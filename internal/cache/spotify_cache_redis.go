...
func (r *RedisSpotifyCache) SearchTracks(accessToken, query string) ([]spotify_api.TrackSearch, error) {
	...
	if err == nil {
		...
		var tracks []spotify_api.TrackSearch
		if err := json.Unmarshal([]byte(val), &tracks); err != nil {
			return []spotify_api.TrackSearch{}, fmt.Errorf("could not unmarshal tracks for query: %v, err %v", query, err)
		}
		return tracks, nil
	}

	if err != redis.Nil {
		return []spotify_api.TrackSearch{}, fmt.Errorf("could not get tracks for query: %v, err %v", query, err)
	}

	// cache miss
	trackList, err := spotify_api.SearchTracksByName(accessToken, query)
	if err != nil {
		return []spotify_api.TrackSearch{}, fmt.Errorf("could not search tracks: %v, err %v", query, err)
	}
	// Store cache
	tracklistJson, err := json.Marshal(trackList)
	if err != nil {
		return []spotify_api.TrackSearch{}, fmt.Errorf("could not marshal searched tracks for: %v, err %v", query, err)
	}
	if err := r.client.Set(r.ctx, key, tracklistJson, r.ttl).Err(); err != nil {
...
func (r *RedisSpotifyCache) SearchAlbums(accessToken, query string) ([]spotify_api.AlbumSearch, error) {
	...
	if err == nil {
		...
		var albums []spotify_api.AlbumSearch
		if err := json.Unmarshal([]byte(val), &albums); err != nil {
			return []spotify_api.AlbumSearch{}, fmt.Errorf("could not unmarshal albums for query: %v, err %v", query, err)
		}
		return albums, nil
	}

	if err != redis.Nil {
		return []spotify_api.AlbumSearch{}, fmt.Errorf("could not get albums for query: %v, err %v", query, err)
	}

	// cache miss
	albumList, err := spotify_api.SearchAlbumsByName(accessToken, query)
	if err != nil {
		return []spotify_api.AlbumSearch{}, fmt.Errorf("could not search albums: %v, err %v", query, err)
	}
	// Store cache
	albumlistJson, err := json.Marshal(albumList)
	if err != nil {
		return []spotify_api.AlbumSearch{}, fmt.Errorf("could not marshal searched albums for: %v, err %v", query, err)
	}
	if err := r.client.Set(r.ctx, key, albumlistJson, r.ttl).Err(); err != nil {
...
func (r *RedisSpotifyCache) SearchArtists(accessToken, query string) ([]spotify_api.ArtistSearch, error) {
	...
	if err == nil {
		...
		var artists []spotify_api.ArtistSearch
		if err := json.Unmarshal([]byte(val), &artists); err != nil {
			return []spotify_api.ArtistSearch{}, fmt.Errorf("could not unmarshal artists for query: %v, err %v", query, err)
		}
		return artists, nil
	}

	if err != redis.Nil {
		return []spotify_api.ArtistSearch{}, fmt.Errorf("could not get artists for query: %v, err %v", query, err)
	}

	// cache miss
	artistList, err := spotify_api.SearchArtistsByName(accessToken, query)
	if err != nil {
		return []spotify_api.ArtistSearch{}, fmt.Errorf("could not search artists: %v, err %v", query, err)
	}
	// Store cache
	artistlistJson, err := json.Marshal(artistList)
	if err != nil {
		return []spotify_api.ArtistSearch{}, fmt.Errorf("could not marshal searched artists for: %v, err %v", query, err)
	}
...
func (r *RedisSpotifyCache) SearchPlaylists(accessToken, query string) ([]spotify_api.PlaylistSearch, error) {
	...
	if err == nil {
		...
		var playlists []spotify_api.PlaylistSearch
		if err := json.Unmarshal([]byte(val), &playlists); err != nil {
			return []spotify_api.PlaylistSearch{}, fmt.Errorf("could not unmarshal playlists for query: %v, err %v", query, err)
		}
		return playlists, nil
	}

	if err != redis.Nil {
		return []spotify_api.PlaylistSearch{}, fmt.Errorf("could not get playlists for query: %v, err %v", query, err)
	}

	// cache miss
	playlistList, err := spotify_api.SearchPlaylistsByName(accessToken, query)
	if err != nil {
		return []spotify_api.PlaylistSearch{}, fmt.Errorf("could not search playlists: %v, err %v", query, err)
	}
	// Store cache
	playlistlistJson, err := json.Marshal(playlistList)
	if err != nil {
		return []spotify_api.PlaylistSearch{}, fmt.Errorf("could not marshal searched playlists for: %v, err %v", query, err)
	}
